...
class App {
  ...
  isDown = false;
  start = 0;
  constructor(container: HTMLElement, { items, bend = 1, textColor = "#ffffff", borderRadius = 0, font = "bold 30px Inter", scrollSpeed = 2, scrollEase = 0.05 }: any) {
  ...
  onTouchDown(e: any) { this.isDown = true; this.scroll.position = this.scroll.current; this.start = "touches" in e ? e.touches[0].clientX : e.clientX; }
  onTouchMove(e: any) { if (!this.isDown) return; const x = "touches" in e ? e.touches[0].clientX : e.clientX; const d = (this.start - x) * (this.scrollSpeed * 0.025); this.scroll.target = (this.scroll.position ?? 0) + d; }
